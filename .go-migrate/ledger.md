...
- [ ] S1 flat session layout `<state>/sessions/<id>.jsonl` (id = existing random hex; no time in name — sort by ctime/created_at); header gains `agentfile` (abs path) and `label`; `workflow_name` stays = Agentfile NAME; label = deployment identity (serve: instance/display name; run: only via --session-label). serve's handleSessionLogs filters by header label, not directory. Old nested dirs still readable (recursive walk), never written.
- [ ] S2 unified `agent replay`: no args ⇒ list table (id, created, name, label, status, duration); `<id|prefix|path|dir>...` ⇒ replay (ambiguous prefix ⇒ candidates, exit 1); selectors `--last`, `--name`, `--agentfile`, `--label`, `--status`, `--since`; `--list` forces the table; multiple matches replay in order; state dir via config precedence + `--state`. Existing flags unchanged; cmd/replay binary gets the same.
