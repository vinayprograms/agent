...
- [ ] C1 config dir `~/.config/grid` → `~/.config/agent` (config.DefaultConfigDir, credentials.StandardPaths("agent"), setup, docs/README, examples). USER RULING: no support for the old `~/.config/grid` directory in any way (no fallback, no warning).
