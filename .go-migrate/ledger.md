...
- [x] R1 (as R1a+R1b) cross-cutting sweep: interface{}→any (X6); one DefaultStateDir/config-dir owner in config (X3/X4, bug 12); setup.Step→Screen (X7); dead-code delete list (diagnosis "Delete" table, re-verify zero callers); config.GetProfile BaseURL/Thinking bug (1); cmd/replay *.json→*.jsonl (6); agentmem Walk nil-deref (8).
...
- [x] R6 cmd/agent → internal/runtime (Load+New+Run) + internal/serve (lifecycle type; status/currentTask sync; one executor per task or serialized; taskDone buffered); bug 2 (cleanup on exit), 11; output via cmd.OutOrStdout/ErrOrStderr (X9); os.Exit only in main (X10, typed exitError); main thin; toolset tests; simple-memory.agent memory_forget fix; 100% of non-TTY code.
Wave 5:
- [x] R7 cmd/replay → delete (Makefile alias) or 5-line main over shared cobra factory; cmd/agentmem cobra; parseCostSpec → replay.ParsePricing; X1 truncate dedupe.
...
- [x] R10 system tests: run every examples/agent/*.agent that needs no MCP/network-only deps via agent.ollama.toml against testdata expected criteria; record results in test-results/; fix Agentfile drift.
...
## Phase 3 — LAST item (after all other work): interactive step debugger
- [x] R11 `agent run --step` (name TBD via naming gauntlet; `--debug` already means verbose logging — do NOT overload it): after each Agentfile step executes, pause and present a yes/no prompt on the CLI ("continue?"); "no" aborts the run cleanly (session footer written, cleanup run). Only valid where an interactive CLI/TUI controls the agent and a human is present: `agent run` on a TTY. Must be rejected with a clear error in `serve`, swarm-dispatched tasks, non-TTY stdin, and any headless mode. Design first (Design-Before-Implementation): the pause is an executor seam (e.g. a `StepGate func(ctx, step) (bool, error)` in RunOptions — consumer-defined, nil ⇒ no pause), wired by cmd/agent only when stdin is a terminal; decide whether sub-agent/converge iterations count as "lines". TDD: executor tests with a fake gate (continue / abort / ctx cancel); cmd/agent tests with a scripted reader; doc in docs/usage/cli-reference.md.
### R9c internal/setup — DONE (7528eac; verifier PASS)
split into model/screens/views/generate/write/probe; Step→Screen; New(opts: WithDir/WithContext); Run(tea.ProgramOption...); m.err cleared on retry; slices.Sort; dead cursor clamps removed; 0600 hint; fresh-mode legacy policy warning. TOML stays string-built (comments). coverage 100%.
...
### R7 CLI unification (minus swarm) — DONE (8c84006, 77e97d2; verifier PASS)
...
R7 follow-ups for R1b: restore agent-replay Long help (nav keys j/k g/G f q, --cost example) + usage on no-args; ParsePricing package-level tests (all 5 error branches); agentmem WalkDir error paths + graph unmarshal error tests; fmt.Errorf-without-verbs → errors.New; sentinel compared with errors.Is; X10 residual handled by R6 (main.go:99, setup.go:14); gofmt internal/replay/stats.go.
### R1a follow-ups (leaf packages) — DONE (bfe0ee0…0bde565; light verification only — items were verifier-specified)
agentfile ValidationErrors type (header restored, Unwrap []error), dead lexer branches gone → 100%; setup Dir(d) + positional ctx, Screen vocabulary → 100%; replay pager ProgramOption passthrough → 92.2%; packaging error wording; skills.Discover deleted; checkpoint QueryEscape; websearch ErrNoProvider hint in doc; session concurrent test + doc.
### R10-fix — DONE on ref/r10 (2325b52…dd57e99 + 9b0a799; verifier PASS; 67/67 edited examples were invalid under the pre-migration parser — no regression; merge after R6)
converge keeps content above CONVERGED; free goal-clause order + TestExamples_ParseAndValidate (all examples/**/*.agent validate); Budget{MaxToolCalls,MaxTurns,MaxDuration} per goal via [limits] (docs/configuration/limits.md; bad max_duration rejected at load); goal.complete once per goal; REQUIRES warnings; GoalName in supervisor log; detach() 2m deadline.
follow-ups for R1b: budget-stop partial output should fall back to last non-empty content; distinguish budget stop from "did not converge"; dedupe N budget warnings across parallel sub-agents; mention 2m background deadline in limits.md; declared `-> var` outputs still absent from printed Outputs (state.Outputs never receives e.outputs) — investigate.
### R6 cmd/agent → internal/run — DONE (d3d1d24, debdbe2, c654a74, fa9a34c; verifier FAIL→fixed: HTTP/bus drain via awaitIdle on exec mutex, bus tasks run under WithoutCancel with drain deadline, Deps.Metrics)
internal/run: Load(LoadOptions) (*Loaded, error); New(ctx, *Loaded, Deps{Creds,Stdout,Stderr,Version,Sink,KeepSession,Metrics}) (*Runtime, error); Run(ctx) error; Close(). cmd/agent: newRootCmd(deps) cobra factory, runner seam, main = NotifyContext+ExecuteContext+sole os.Exit (defer Close runs on failure); serve.go in place (mutex state, serialised exec, http timeouts, MaxBytesReader, idle corrections as session warning). CLI parity byte-identical (help diff), Ctrl-C now cancels and writes footer. coverage cmd/agent 60%, internal/run 88.3%.
### R10-fix — MERGED (conflicts in internal/run/runtime.go resolved: REQUIRES warning via rt.stderr).

### R11 design (deliberated; implement after R1b + rerun land)
- Granularity: "each line" = each `RUN` step; a RUN with several goals runs them sequentially, so the gate fires after EACH goal inside a RUN step (the unit of real work), labelled "RUN <step> › goal <name>". Sub-agent spawns and CONVERGE iterations are not gated (they are inside one goal) — documented.
- Seam (consumer-defined, nil ⇒ off): `executor.RunOptions.StepGate func(ctx context.Context, step, goal string) (proceed bool, err error)`; called by the executor's GoalExecutor path right after a goal completes (success or failure); `proceed=false` ⇒ `Run` returns `ErrAborted` (sentinel) with status "aborted" in the session footer, cleanup runs as usual. Tests: fake gate continue / abort / ctx-cancel / gate error, footer status, no gate ⇒ unchanged.
- CLI: `agent run --step` (gauntlet: `--step` > `--pause`/`--interactive`; `--debug` stays verbose-logging). Only on `run`; `serve` has no such flag; swarm-dispatched tasks never see it. `run --step` with non-TTY stdin (or stdout) ⇒ error "--step needs an interactive terminal". Gate impl in cmd/agent: prints "RUN %s › goal %q finished. Continue? [Y/n] " to stderr, reads a line from stdin (bufio), y/enter ⇒ proceed, n ⇒ abort; EOF ⇒ abort. Tests: scripted reader + buffer; TTY check injected via deps.
- Docs: docs/usage/cli-reference.md gets `--step`; README one line.
...
- [x] C1 (d4d7f3f) config dir `~/.config/grid` → `~/.config/agent` (config.DefaultConfigDir, credentials.StandardPaths("agent"), setup, docs/README, examples). USER RULING: no support for the old `~/.config/grid` directory in any way (no fallback, no warning).
- [x] C2 (f8991b5) symmetry: default-policy fallback `~/.config/agent/policy.toml` (then permissive+warning); `--credentials PATH` on run/serve → run.LoadOptions.CredentialsPath (highest precedence, must exist).
- [x] C3 (08d8bb9…8cae941, verifier FAIL→fixed: credentials --force guard, structural redaction) `agent config {init,show,validate,path}` with `--dir PATH | --default` (exclusive; neither ⇒ cwd). init: flags --provider --model --scenario --force, writes agent.toml+policy.toml (0644) + credentials.toml (0600, only with --api-key or env); generators shared with setup (extract to internal/configfile). show: effective values + source file per section, secrets redacted, --resolved. validate: schema + legacy keys + provider-without-credential. path: resolved paths. `setup` gains --dir/--default; edit mode must pre-fill from that location. No `set` (user decision).
- [x] C4 (d6fcf87 — docs/configuration/README.md, validated examples) docs/configuration/README.md Configuration Guide (three files, lookup table, minimal→annotated reference, providers, profiles/REQUIRES, limits, security modes, legacy-key migration, credential hygiene); linked from README + cli-reference.

## Phase 5 — sessions (user request 2026-08-22)
- [x] S1 (2831350, verifier PASS) flat session layout `<state>/sessions/<id>.jsonl` (id = existing random hex; no time in name — sort by ctime/created_at); header gains `agentfile` (abs path) and `label`; `workflow_name` stays = Agentfile NAME; label = deployment identity (serve: instance/display name; run: only via --session-label). serve's handleSessionLogs filters by header label, not directory. Old nested dirs still readable (recursive walk), never written.
- [x] S2 (1a5fffe, f065f2e, 36be129; verifier PASS) unified `agent replay`: no args ⇒ list table (id, created, name, label, status, duration); `<id|prefix|path|dir>...` ⇒ replay (ambiguous prefix ⇒ candidates, exit 1); selectors `--last`, `--name`, `--agentfile`, `--label`, `--status`, `--since`; `--list` forces the table; multiple matches replay in order; state dir via config precedence + `--state`. Existing flags unchanged; cmd/replay binary gets the same.
User ruling 2026-08-22: keep the separate `agent-replay` (cmd/replay) and `agentmem` binaries as conveniences over the shared command code; do NOT fold them into `agent`.
### W1 — DONE (1f1f4b5, 73a840f, 9a4bd89, 5edbd37; verifier PASS after proxy-preserving transport + real h2 test): internal/tools/webfetch (HTTP/1.1, browser headers, kit parity), internal/tools/httpclient shared transport, websearch WithCooldown + 5-min cache + jitter; [timeouts] search_cooldown_ms wired.
### W2 — DONE (e96fa9b…6a01e01; verifier PASS): [web] search_provider/searxng_url decoded+validated+wired (pinned searxng without URL fails at startup); session tool_result meta.error (+meta.result in --debug); Config.UnknownKeys → run WARN / validate exit 1; web_search output "Source: <provider>", errors "<provider>: …"; config validate warns on keyless DDG fallback; TestToolsetConfig_WiredFromConfig guards future keys. Root cause of user sessions 58baff71/8e6f747f: [web] keys were never read (pre-existing), DDG 403; SearXNG needed search.formats json.
