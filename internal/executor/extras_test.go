...
func TestSecurityResearchPrefix(t *testing.T) {
	...
	_, _, _ = exec.spawnAgentWithPrompt(context.Background(), "r", "sys", "t", nil, "", []GoalOutput{{ID: "prev", Output: "o"}}, false, 0)
...
func TestGuidancePrefixes_ForSubAgents(t *testing.T) {
	...
	if _, _, err := exec.spawnAgentWithPrompt(context.Background(), "r", "sys", "t", []string{"f"}, "", nil, false, 0); err != nil {
		t.Fatal(err)
	}
